...
st.markdown("<h1 style = 'color: #D39D55; text-align: center; font-size: 60px; font-family: Times New Roman'>AUTOMOBILE PRICE PREDICTION</h1>", unsafe_allow_html = True)
st.markdown("<h4 style = 'margin: -30px; color: #131010; text-align: center; font-family: Serif '>~Built by Kabiru Abdullahi~</h4>", unsafe_allow_html = True)
st.markdown("<br>", unsafe_allow_html=True)

#add an image
st.image('automobile_image.png', caption = 'Built by Kabiru')
...
st.markdown("Automobile price prediction involves using machine learning models to estimate the price of a vehicle based on various features such as brand, model, year, mileage, fuel type, engine size, and other attributes. By analyzing historical pricing data and incorporating factors like market trends, economic indicators, and consumer preferences, the research seeks to develop a predictive model that can assist buyers, sellers, and industry professionals in making informed decisions. This study not only contributes to enhancing pricing strategies in the automotive market but also provides valuable insights into the factors influencing vehicle prices in a dynamic and competitive industry.</p>", unsafe_allow_html=True)

# Sidebar design (to put what you want on the side)
st.sidebar.image('automobile_user_image.png')
